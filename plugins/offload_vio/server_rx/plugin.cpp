...
server_reader::server_reader(const std::string& name_, phonebook* pb_)
...
threadloop::skip_option server_reader::_p_should_skip() {
...
void server_reader::ReceiveVioInput(const vio_input_proto::IMUCamVec& vio_input) {
    ...
    for (int i = 0; i < vio_input.imu_data_size() - 1; i++) {
        ...
        _m_imu.put(_m_imu.allocate<imu_type>(imu_type{
                ...
                Eigen::Vector3d{curr_data.angular_vel().x(), curr_data.angular_vel().y(),
                                curr_data.angular_vel().z()},
                Eigen::Vector3d{curr_data.linear_accel().x(), curr_data.linear_accel().y(),
                                curr_data.linear_accel().z()}}));
        }
    ...
    _m_imu.put(_m_imu.allocate<imu_type>(
            imu_type{time_point{std::chrono::nanoseconds{last_imu.timestamp()}},
                     Eigen::Vector3d{last_imu.angular_vel().x(), last_imu.angular_vel().y(),
                                     last_imu.angular_vel().z()},
                     Eigen::Vector3d{last_imu.linear_accel().x(), last_imu.linear_accel().y(),
                                     last_imu.linear_accel().z()}}));
}

...
server_writer::server_writer(const std::string& name_, phonebook* pb_)
        : plugin{name_, pb_}
...
void server_writer::start() {
    ...
    sb->schedule<pose_type>(id, "slow_pose", [this](const switchboard::ptr<const pose_type>& datum,
            std::size_t) {
...
void server_writer::start_accepting_connection(switchboard::ptr<const connection_signal>& datum) {
    socket.listen();
#ifndef NDEBUG

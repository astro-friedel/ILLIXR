...
offload_reader::offload_reader(const std::string& name_, phonebook* pb_)
...
threadloop::skip_option offload_reader::_p_should_skip() {
...
void offload_reader::ReceiveVioOutput(const vio_output_proto::VIOOutput& vio_output,
    ...
    pose_type datum_pose_tmp{
            ...
            Eigen::Vector3f{static_cast<float>(slow_pose.position().x()),
                            static_cast<float>(slow_pose.position().y()),
                            static_cast<float>(slow_pose.position().z())},
            Eigen::Quaternionf{static_cast<float>(slow_pose.rotation().w()),
    ...
    imu_integrator_input datum_imu_int_tmp{
            ...
            Eigen::Vector3d{imu_int_input.biasacc().x(), imu_int_input.biasacc().y(),
                            imu_int_input.biasacc().z()},
            Eigen::Vector3d{imu_int_input.biasgyro().x(), imu_int_input.biasgyro().y(),
                            imu_int_input.biasgyro().z()},
            ...
            Eigen::Quaterniond{imu_int_input.rotation().w(), imu_int_input.rotation().x(),
                               imu_int_input.rotation().y(),

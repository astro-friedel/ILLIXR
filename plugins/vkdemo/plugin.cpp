...
void vkdemo::record_command_buffer(VkCommandBuffer commandBuffer, int eye) {
    ...
    vkCmdBindDescriptorSets(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, pipeline_layout, 0, 1,
                            &descriptor_sets[eye], 0, nullptr);

    for (auto& model : models) {
        VKdemo::ModelPushConstant push_constant{};
        ...
        vkCmdPushConstants(commandBuffer, pipeline_layout, VK_SHADER_STAGE_FRAGMENT_BIT, 0,
                           sizeof(VKdemo::ModelPushConstant), &push_constant);
        vkCmdDrawIndexed(commandBuffer, model.index_count, 1, model.index_offset, 0, 0);
    }
}

void vkdemo::update_uniform(const pose_type& fp, int eye) {
    ...
    auto* ubo = (VKdemo::UniformBufferObject*) uniform_buffer_allocation_infos[eye].pMappedData;
...
void vkdemo::create_descriptor_set_layout() {
    ...
    VK_ASSERT_SUCCESS(vkCreateDescriptorSetLayout(ds->vk_device, &layout_info, nullptr,
...
void vkdemo::create_uniform_buffers() {
    for (auto i = 0; i < 2; i++) {
        VkBufferCreateInfo buffer_info{
                ...
                nullptr,                              // pNext
                0,                                    // flags
                sizeof(VKdemo::UniformBufferObject),          // size
...
void vkdemo::create_texture_sampler() {
...
void vkdemo::create_descriptor_set() {
    ...
    std::array<VkDescriptorBufferInfo, 2> buffer_infos = {{{
                                                               ...
                                                               0,                          // offset
                                                               sizeof(VKdemo::UniformBufferObject) // range
                                                           },
                                                           {
    ...
    vkUpdateDescriptorSets(ds->vk_device, static_cast<uint32_t>(descriptor_writes.size()),
...
void vkdemo::load_texture(const std::string& path, int i) {
...
#ifndef NDEBUG
    spdlog::get("illixr")->debug("[vkdemo] Loaded texture {} with dimensions {}x{} and {} channels", path,
    ...
    VK_ASSERT_SUCCESS(vmaCreateBuffer(vma_allocator, &buffer_info, &alloc_info, &staging_buffer,
...
void vkdemo::image_layout_transition(VkImage image, [[maybe_unused]] VkFormat format,
    ...
    vkCmdPipelineBarrier(command_buffer_local, source_stage, destination_stage, 0, 0, nullptr, 0, nullptr,
                         1, &barrier);
...
void vkdemo::load_model() {
    ...
    std::unordered_map<VKdemo::Vertex, uint32_t> unique_vertices{};
    for (const auto& shape : shapes) {
        VKdemo::Model model{};
        model.index_offset = static_cast<uint32_t>(indices.size());
        for (const auto& index : shape.mesh.indices) {
            VKdemo::Vertex vertex{};
...
void vkdemo::create_index_buffer() {
    ...
    VkBuffer      staging_buffer;
    VmaAllocation staging_buffer_allocation;
    VK_ASSERT_SUCCESS(vmaCreateBuffer(vma_allocator, &staging_buffer_info, &staging_alloc_info,
    ...
    VmaAllocation buffer_allocation;
    VK_ASSERT_SUCCESS(vmaCreateBuffer(vma_allocator, &buffer_info, &alloc_info, &index_buffer,
...
void vkdemo::create_pipeline(VkRenderPass render_pass, uint32_t subpass) {
    ...
    auto binding_description    = VKdemo::Vertex::get_binding_description();
    auto attribute_descriptions = VKdemo::Vertex::get_attribute_descriptions();
    ...
    VkPushConstantRange push_constant_range{
        VK_SHADER_STAGE_FRAGMENT_BIT, // stageFlags
        0,                            // offset
        sizeof(VKdemo::ModelPushConstant)     // size
    };
    ...
    VK_ASSERT_SUCCESS(vkCreateGraphicsPipelines(ds->vk_device, VK_NULL_HANDLE, 1, &pipeline_info, nullptr,
                                                &pipeline))
...
vkdemo_plugin::vkdemo_plugin(const std::string& name, phonebook* pb)
        : plugin{name, pb}

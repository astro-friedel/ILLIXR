...
GLubyte* timewarp_gl::readTextureImage() {
...
GLuint timewarp_gl::ConvertVkFormatToGL(int64_t vk_format) {
    switch (vk_format) {
...
void timewarp_gl::ImportVulkanImage(const vk_image_handle& vk_handle, swapchain_usage usage) {
...
void timewarp_gl::BuildTimewarp(HMD::hmd_info_t& hmdInfo) {
...
void timewarp_gl::CalculateTimeWarpTransform(Eigen::Matrix4f& transform, const Eigen::Matrix4f& renderProjectionMatrix,
...
time_point timewarp_gl::GetNextSwapTimeEstimate() {
...
duration timewarp_gl::EstimateTimeToSleep(double framePercentage) {
    ...
    if (!rendering_ready) {
        ...
        for (int eye = 0; eye < 2; eye++) {
            ...
            glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB16F, display_params::width_pixels * 0.5f,
                         display_params::height_pixels, 0,
...
void timewarp_gl::warp(switchboard::ptr<const rendered_frame>& most_recent_frame) {
    if (!rendering_ready)
        _prepare_rendering();
    ...
    glUniform1i(static_cast<GLint>(flip_y_unif), false);

...
VkVertexInputBindingDescription Timewarp_VK::Vertex::get_binding_description() {
...
std::array<VkVertexInputAttributeDescription, 4> Timewarp_VK::Vertex::get_attribute_descriptions() {
...
void timewarp_vk::update_uniforms(const pose_type& render_pose) {
    ...
    auto* ubo = (Timewarp_VK::UniformBufferObject*) uniform_alloc_info.pMappedData;
...
void timewarp_vk::record_command_buffer(VkCommandBuffer commandBuffer, int buffer_ind, bool left) {
    num_record_calls++;
    ...
    vkCmdDrawIndexed(commandBuffer, num_distortion_indices, 1, 0,
                     static_cast<int>(num_distortion_vertices * !left), 0);
}
...
void timewarp_vk::create_vertex_buffer() {
    ...
    staging_buffer_info.size  = sizeof(Timewarp_VK::Vertex) * num_distortion_vertices * HMD::NUM_EYES;
    ...
    VK_ASSERT_SUCCESS(
        vmaCreateBuffer(vma_allocator, &staging_buffer_info, &staging_alloc_info, &staging_buffer,
    ...
    buffer_info.size  = sizeof(Timewarp_VK::Vertex) * num_distortion_vertices * HMD::NUM_EYES;
    ...
    VK_ASSERT_SUCCESS(vmaCreateBuffer(vma_allocator, &buffer_info, &alloc_info, &vertex_buffer,
                                      &vertex_alloc, nullptr))

    std::vector<Timewarp_VK::Vertex> vertices;
    ...
    memcpy(mapped_data, vertices.data(), sizeof(Timewarp_VK::Vertex) * num_distortion_vertices * HMD::NUM_EYES);
    ...
    copy_region.size                     = sizeof(Timewarp_VK::Vertex) * num_distortion_vertices * HMD::NUM_EYES;
...
void timewarp_vk::create_index_buffer() {
    ...
    VK_ASSERT_SUCCESS(vmaCreateBuffer(vma_allocator, &buffer_info, &alloc_info, &index_buffer, &index_alloc,
                                      nullptr))
...
void timewarp_vk::create_descriptor_set_layout() {
    ...
    VK_ASSERT_SUCCESS(vkCreateDescriptorSetLayout(ds->vk_device, &layoutInfo, nullptr,
...
void timewarp_vk::create_uniform_buffer() {
    ...
    bufferInfo.size  = sizeof(Timewarp_VK::UniformBufferObject);
    ...
    VK_ASSERT_SUCCESS(
        vmaCreateBuffer(vma_allocator, &bufferInfo, &createInfo, &uniform_buffer, &uniform_alloc,
                        &uniform_alloc_info))
    deletion_queue.emplace([=]() {
...
void timewarp_vk::create_descriptor_sets() {
    ...
    for (int eye = 0; eye < 2; eye++) {
        ...
        for (size_t i = 0; i < buffer_pool[0].size(); i++) {
            ...
            bufferInfo.offset                 = 0;
            bufferInfo.range                  = sizeof(Timewarp_VK::UniformBufferObject);
...
VkPipeline timewarp_vk::create_pipeline(VkRenderPass render_pass, [[maybe_unused]] uint32_t subpass) {
    ...
    auto bindingDescription    = Timewarp_VK::Vertex::get_binding_description();
    auto attributeDescriptions = Timewarp_VK::Vertex::get_attribute_descriptions();
...
void timewarp_vk::build_timewarp(HMD::hmd_info_t& hmdInfo) {
    ...
    math_util::projection_fov(&basicProjection, display_params::fov_x / 2.0f, display_params::fov_x / 2.0f,
                              ...
                              rendering_params::near_z, rendering_params::far_z);
}

/* Calculate timewarm transform from projection matrix, view matrix, etc */
void timewarp_vk::calculate_timewarp_transform(Eigen::Matrix4f& transform, const Eigen::Matrix4f& renderProjectionMatrix,
...
timewarp_vk_plugin::timewarp_vk_plugin(const std::string& name, phonebook* pb)
        : threadloop{name, pb}

...
std::shared_ptr<Camera> ILLIXR::start_camera() {
...
zed_camera_thread::zed_camera_thread(const std::string& name_, phonebook* pb_, std::shared_ptr<Camera> zedm_)
...
void zed_imu_thread::_p_one_iteration() {
    ...
    if (c && c->serial_no != last_serial_no) {
        _m_cam_publisher.put(_m_cam_publisher.allocate<cam_type>({imu_time_point, cv::Mat{c->img0},
                                                                  cv::Mat{c->img1}}));
        _m_rgb_depth.put(_m_rgb_depth.allocate<rgb_depth_type>({imu_time_point, cv::Mat{c->rgb},
                                                                cv::Mat{c->depth}}));

...
gtsam_integrator::gtsam_integrator(const std::string& name_, phonebook* pb_)
        : plugin{name_, pb_}
...
void gtsam_integrator::PimObject::resetIntegrationAndSetBias(const imu_int_t& imu_int_input) noexcept {
    assert(_pim != nullptr && "_pim should not be null");
...
void gtsam_integrator::PimObject::integrateMeasurement(const imu_t& imu_input, const imu_t& imu_input_next) noexcept {
...
[[nodiscard]] bias_t gtsam_integrator::PimObject::biasHat() const noexcept {
    assert(_pim != nullptr && "_pim shuold not be null");
    return _pim->biasHat();
}

[[nodiscard]] nav_t gtsam_integrator::PimObject::predict() const noexcept {
    assert(_pim != nullptr && "_pim should not be null");
...
std::vector<imu_type> gtsam_integrator::select_imu_readings(const std::vector<imu_type>& imu_data,
...
imu_type gtsam_integrator::interpolate_imu(const imu_type& imu_1, const imu_type& imu_2,

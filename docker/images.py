docker_images = {"CentOS": {"9": {"image": "quay.io/centos/centos:stream9",
    ...
    for op in ops:
        for osc in opers:
            if op.upper() == osc.upper():
                to_build.union(set([f"{osc}-{y}" for y in docker_images[osc]]))
                found = True
                continue
        if found:
            continue
